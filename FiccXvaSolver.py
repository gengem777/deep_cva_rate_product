...
class FiccXvaSolver(object):
    ...
    def train(self):
        '''
        train the control problem with loss function loss_fn and print the evaluation of model with validate data.
        '''
    ...
    def loss_fn(self, inputs, training):
        '''
        define the MSE loss function between terminal value and exact terminal payoff. The loss function has been clipped at large value space. now the clean value sample are also in the input
        '''
        dw, r, B, P, v_clean, coll = inputs
        y_terminal = self.model(inputs, training)
        delta = y_terminal - self.bsde.g_tf(self.bsde.total_time, P[:, :, -1], v_clean[:, :, -1], coll[:, :, -1])
...
class NonsharedModel(tf.keras.Model):
    ...
    def call(self, inputs, training):
        '''
        forward propagate (under training set)
        '''
        dw, r, B, P, v_clean, coll = inputs
        ...
        for t in range(0, self.bsde.num_time_interval - 1):
            y = y - self.bsde.delta_t * (
                self.bsde.f_tf(time_stamp[t], r[:, :, t], P[:, :, t], y, z, v_clean[:, :, t], coll[:, :, t])
            ) + tf.reduce_sum(z * dw[:, :, t], 1, keepdims=True)
            try:
                z = self.subnet[t](P [:, :, t + 1], training) / self.bsde.dim
            except TypeError:
                z = self.subnet(tf.concat([time_stamp[t + 1] * all_one_vec, P[:, :, t + 1]], axis=1),
                                training=training) / self.bsde.dim
        # terminal time
        y = y - self.bsde.delta_t * self.bsde.f_tf(time_stamp[-1], r[:, :, -2], P[:, :, -2], y, z, v_clean[:, :, -2],
                                                   coll[:, :, -2]) + \
    ...
    def predict_step(self, data):
        '''
        forward propagate (under validating set)
        '''
        print(data[0])
        dw, r, B, P, v_clean, coll = data[0]
        time_stamp = np.arange(0, self.eqn_config.num_time_interval) * self.bsde.delta_t
        all_one_vec = tf.ones(shape=tf.stack([tf.shape(dw)[0], 1]), dtype=self.net_config.dtype)
        y = all_one_vec * self.y_init
        z = tf.matmul(all_one_vec, self.z_init)
        ...
        for t in range(0, self.bsde.num_time_interval - 1):
            ...
            try:
                z = self.subnet[t](P[:, :, t + 1], training=False) / self.bsde.dim
            except TypeError:
                z = self.subnet(tf.concat([time_stamp[t + 1] * all_one_vec, P[:, :, t + 1]], axis=1),
                                training=False) / self.bsde.dim
        # terminal time
        y = y - self.bsde.delta_t * self.bsde.f_tf(time_stamp[-1], r[:, :, -2], P[:, :, -2], y, z, v_clean[:, :, -2],
                                                   coll[:, :, -2]) + \
            tf.reduce_sum(z * dw[:, :, -1], 1, keepdims=True)
        ...
        return dw, P, v_clean, coll, history
...
class FeedForwardSubNet(tf.keras.Model):
    '''
    this class gives the subnet of the diffusion, which is a FNN with batchnormalization
    '''
