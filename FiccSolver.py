...
class BSDESolver(object):
    ...
    def loss_fn(self, inputs, training):
        '''
        define the MSE loss function between terminal value and exact terminal payoff. The loss function has been clipped at large value space.
        '''
        dw, r, B, P = inputs
        y_terminal = self.model(inputs, training)
        delta = y_terminal - self.bsde.g_tf(self.bsde.total_time, P[:, :, -1])
        # use linear approximation outside the clipped range
        loss = tf.reduce_mean(tf.where(tf.abs(delta) < DELTA_CLIP, tf.square(delta),
                                       2 * DELTA_CLIP * tf.abs(delta) - DELTA_CLIP ** 2))

        loss += 1000 * (tf.maximum(self.model.y_init[0] - self.net_config.y_init_range[1], 0) + tf.maximum(
            self.net_config.y_init_range[0] - self.model.y_init[0], 0))
        return loss
    ...
    def call(self, inputs, training):
        ...
        for t in range(0, self.bsde.num_time_interval - 1):
            y = y - self.bsde.delta_t * (
                self.bsde.f_tf(time_stamp[t], r[:, :, t], P[:, :, t], y, z)
        ...
        # terminal time
        y = y - self.bsde.delta_t * self.bsde.f_tf(time_stamp[-1], r[:, :, -2], P[:, :, -2], y, z) + \
